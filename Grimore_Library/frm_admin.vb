...
Public Class frm_admin
    ...
    Sub Carregar_Admin(pesquisa As String)
        Try
            If pesquisa = "" Then
                SQL = "SELECT matricula, senha, pergunta_seg, status_conta, usuario, tipo_usuario FROM tb_usuarios ORDER BY usuario ASC"
            Else
                SQL = $"SELECT matricula, senha, pergunta_seg, status_conta, usuario, tipo_usuario FROM tb_usuarios WHERE usuario LIKE '%{pesquisa}%' ORDER BY usuario ASC"
            End If
            ...
            With dgv_admin
                ...
                Do While rs.EOF = False
                    .Rows.Add(
                        rs.Fields("usuario").Value.ToString(),        ' NOME
                        rs.Fields("matricula").Value.ToString(),      ' MATRICULA
                        rs.Fields("senha").Value.ToString(),          ' SENHA
                        rs.Fields("pergunta_seg").Value.ToString(),   ' RESPOSTA
                        rs.Fields("tipo_usuario").Value.ToString(),   ' TIPO
                        rs.Fields("status_conta").Value.ToString(),   ' STATUS
                        Nothing,
                        Nothing
                    )
                    rs.MoveNext()
    ...
    Private Sub btn_buscar_Click(sender As Object, e As EventArgs)
        Try
            SQL = $"SELECT * FROM tb_usuarios where matricula like '%{txt_userbusc.Text}%' order by usuario asc"
            rs = database.Execute(SQL)
            With dgv_admin
                .Rows.Clear()
                Do While rs.EOF = False
    ...
    Private Sub txt_txt_userbusc_TextChanged(sender As Object, e As EventArgs) Handles txt_userbusc.TextChanged
        Carregar_Admin(txt_userbusc.Text)
    End Sub
End Class